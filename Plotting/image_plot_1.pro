...
;stop
  ...
  if COLORBAR_PLACE eq 1 then $ ; vertical, on the right
    color_bar, min=RANGE[0], max=RANGE[1], /vertical, /right, divisions=4, format = CLRBAR_FORMAT,$
              position = [position[2]+0.015, position[1], position[2]+0.03, position[3] ], color=0;!P.color   ;0 for EPS output ?
  ...
  if COLORBAR_PLACE eq 3 then $ ; horizontal, on the bottom
    color_bar, min=RANGE[0], max=RANGE[1],  format = CLRBAR_FORMAT, $
              position = [position[0],position[1]-0.07, position[2], position[1]-0.05 ], color=!P.color   ;0 for EPS output ?
